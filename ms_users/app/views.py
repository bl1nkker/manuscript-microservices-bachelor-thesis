...
def register(request):
    ...
    try:
        ...
        logger.error(request.user, f"POST /register ERROR: {e}")
...
def login(request):
    ...
    try:
        ...
        logger.error(request.user, f"POST /login ERROR: {e}")
        return Response({"message": exceptions.UNKNOWN_EXCEPTION_MESSAGE}, status=400)
...
def users(request, uid: int):
    ...
    try:
        ...
        logger.error(request.user, f"GET /users/{uid} ERROR: {e}")
        return Response({"message": exceptions.UNKNOWN_EXCEPTION_MESSAGE}, status=400)
...
def me(request):
    try:
        ...
        logger.error(request.user, f"GET /me ERROR: {e}")
        return Response({"message": exceptions.UNKNOWN_EXCEPTION_MESSAGE}, status=400)
