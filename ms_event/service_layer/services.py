...
def list_events_service(uow: uow.AbstractUnitOfWork, **kwargs):
    with uow:
        ...
        return Result(data={
            "events": [event.to_dict() for event in events]}, error=None)


def create_event_service(uow: uow.AbstractUnitOfWork, **kwargs):
    with uow:
        if not kwargs.get('name'):
            return Result(data=None, error=exceptions.InvalidEventDataException)
        event = uow.event.create(**kwargs)
        return Result(data=event.to_dict(), error=None)


def edit_event_service(uow: uow.AbstractUnitOfWork, id: int, **kwargs):
    with uow:
        if not kwargs.get('name'):
            return Result(data=None, error=exceptions.InvalidEventDataException)
        event = uow.event.get(id=id)
...
def deactivate_event_service(uow: uow.AbstractUnitOfWork, id: int, uid: int):
    with uow:
        event = uow.event.get(id=id)
        if event is None:
            return Result(data=None, error=exceptions.EventNotFoundException)
